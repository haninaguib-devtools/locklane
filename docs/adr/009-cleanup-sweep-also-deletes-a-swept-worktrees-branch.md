# ADR-009: The cleanup sweep also deletes a swept worktree's local branch — widening the ADR-008 carve-out
...
mechanical, un-deliberate origin ADR-008 already used to justify sweeping the worktree
itself. Once ADR-008's sweep removes the worktree, that branch is exactly the kind of
...
it, the same gap ADR-008 closed for the worktree it lived in. Left alone, these
...
This is a second exception to ADR-005, attached to the same guarded lineage ADR-008
started, not a new independent one: it only ever fires on a branch whose worktree the
sweep *itself just removed* under ADR-008's three-part guard. A branch whose worktree
...
- **Same origin, same argument, one step further than ADR-008 already went.** ADR-008
  ...
  same test ADR-008 applied, not a new one.
...
- **No new judgment, no new failure mode.** The branch delete piggybacks on a removal
  the ADR-008 guard already proved safe (issue closed, worktree clean, unattached); it
...
indefinitely along with its worktree, following ADR-008's own accepted consequence; and
...
1. **`git branch -d` ever deletes a branch that turns out to matter** — this should be
   ...
   guard rather than patching around it, the same posture ADR-008 took for its own
   trigger 1.
...
3. **A worktree-creation path this decision was not written for appears** — e.g. a
   project-console worktree (ADR-008's own trigger 3, #339) gaining a branch of its
