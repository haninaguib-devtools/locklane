# ADR-010: Remove a project-console worktree on tab close, with the sweep as backstop — a second, distinct ADR-008 carve-out
...
ADR-008 carved a narrow, guarded exception out of ADR-005's "left alone permanently,
...
now exists on disk, let alone that it should eventually be removed by hand. ADR-008's
...
almost never commits. ADR-008's guard cannot apply here at all: its first condition is
...
human touchpoint than ADR-008's own target — closing a console tab is a single click
...
"issue closed" — there is no such signal — but it carries a risk ADR-008's guard never
...
ADR-008's three-part guard (issue closed, clean, unattached) has no analog for "does
...
concern ADR-009 addresses for that branch specifically). A detached-HEAD worktree has
...
ADR-008 already established for the per-issue case:

1. the session has ended — no live session's working directory is inside it
   (`SessionRegistry#hasLiveSessionIn`, the same check ADR-008 already uses; at tab
...
3. `git status --porcelain` reports nothing outstanding — the same cleanliness check
   ADR-008 already uses, reused verbatim;
4. HEAD's commit is an ancestor of (freshly fetched) `origin/main` — the guard ADR-008
...
first — the same `removalRefusalReason` pattern ADR-008's own per-issue guard already
established, extended to cover this second, distinct eligibility rule rather than
folding it into the first.

This is a second, independent carve-out from ADR-005, sitting alongside ADR-008's
...
still governed by ADR-008 and ADR-009 exactly as written; nothing here changes either.

## Rationale

- **Same absence-of-human-touchpoint argument ADR-008 already made, for a path ADR-008
  ...
  issue-state signal attached to it at all. ADR-008's own revisit trigger 3 anticipated
...
- **The ancestor-of-`origin/main` check is the one genuinely new piece, and it exists
  ...
  outlives the worktree (modulo ADR-009's own separate, git-merge-gated deletion); a
  ...
  the minimum check that makes automatic removal as safe here as ADR-008's guard
...
- **The sweep is the backstop for exactly the cases the synchronous close-time attempt
  ...
  role ADR-008's sweep already plays for the per-issue path, extended to this second
  eligibility rule rather than duplicated as a separate mechanism.

## Alternatives considered

- **Fold this into ADR-008's existing guard as a fourth, universal condition** —
  rejected: ADR-008's first condition ("issue closed") is meaningless for a worktree
...
on disk exactly as long as ADR-008's own accepted consequence already allows for the
...
1. **This guard ever allows removal of a worktree that turns out to have held
   unrecoverable work** — the single most serious trigger, the same posture ADR-008
   and ADR-009 both take toward their own guard's first condition; suspend the
...
2. **A project-console worktree ever needs its own branch preserved past its
   worktree's removal**, mirroring ADR-009 for the per-issue path — not needed today
...
3. **A third worktree-creation path appears that this guard was not written for** —
   the same shape of gap ADR-008's own trigger 3 named for this one.
