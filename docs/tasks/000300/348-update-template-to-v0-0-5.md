...
## Explicitly not
- No changes to any file outside the template-owned set plus this record.
...
- Update stopped after applying the sync, before push/PR: `consistency-check.sh`
  ...
  local file and finds no matching decision headings. Resolution (likely renumbering
  the local ADR) is outside this task's scope and awaits the human's decision.
