...
import { ProjectAgentSessionComponent } from './project-agent-session.component';
...
describe('ProjectAgentSessionComponent', () => {
  ...
  beforeEach(() => {
    ...
    TestBed.configureTestingModule({
      imports: [ProjectAgentSessionComponent],
  ...
  // Since #537 the page reads the project before its agent sessions. `projects` is what that
  ...
  function init(
    projectId = 1,
    projects: Project[] = [],
  ): ReturnType<typeof TestBed.createComponent<ProjectAgentSessionComponent>> {
    const fixture = TestBed.createComponent(ProjectAgentSessionComponent);
  ...
  function renameField(fixture: ReturnType<typeof TestBed.createComponent<ProjectAgentSessionComponent>>, index = 0) {
  ...
  function typeAndCommit(fixture: ReturnType<typeof TestBed.createComponent<ProjectAgentSessionComponent>>,
  ...
  function openTabMenu(fixture: ReturnType<typeof TestBed.createComponent<ProjectAgentSessionComponent>>, index = 0): void {
    const compiled = fixture.nativeElement as HTMLElement;
  ...
  it('shows a tab per open agent session and selects the most recently attached one', () => {
    const fixture = init();
  ...
  it('activates the agent session named by the agent sessions page\'s ?session handoff (#179)', fakeAsync(() => {
  ...
  it('falls back to the most recently attached agent session when ?session names a closed one', fakeAsync(() => {
  ...
  it('reopens on the tab this browser last selected, over a later-attached agent session (#810)', () => {
  ...
  it('falls back to the most recently attached agent session when the remembered one is no longer open (#810)', () => {
    TestBed.inject(LastAgentSessionStore).set(1, '1-console-gone0000');
  ...
  it('lets a ?session handoff win over the remembered tab (#810)', fakeAsync(() => {
    TestBed.inject(LastAgentSessionStore).set(1, '1-console-e5f6a7b8');
    TestBed.inject(Router).navigateByUrl('/projects/1/console?session=1-console-a1b2c3d4');
    tick();
  ...
  it('shows no Overview tab, and its "+" offers the installed agents before starting anything (#256, #757)', () => {
    ...
    // The chosen agent, not the Settings default, is what the new agent session attaches with.
  ...
  it('keeps every agent session mounted, hiding all but the selected tab', () => {
    const fixture = init();
    httpMock.expectOne('/api/projects/1/console/sessions').flush([
  ...
  it('records the selected tab as this project\'s most recently interacted-with agent session (#221)', () => {
    const fixture = init();
    httpMock.expectOne('/api/projects/1/console/sessions').flush([
      row('1-console-a1b2c3d4', '2026-08-27T11:00:00Z'),
      row('1-console-e5f6a7b8', '2026-08-27T10:00:00Z'),
    ...
    expect(TestBed.inject(LastAgentSessionStore).get(1)).toBe('1-console-e5f6a7b8');
  });
  ...
  it('starts a session with the Settings default agent and then shows the terminal (#256)', () => {
    ...
    expect(TestBed.inject(LastAgentSessionStore).get(1)).toBe('1-console-a1b2c3d4');
    // #194: the header agent sessions widget must learn about it.
    expect(opened).toHaveBeenCalled();
  });

  it('starts an agent session with a real agent, not empty, on a page that never visited Settings or project-summary first (#698)', () => {
  ...
  it('gives each agent session its own working directory from the engine — never a shared checkout (#314)', () => {
    const fixture = init();
    httpMock
  ...
  it('closes an agent session from its tab and falls back to the first remaining one', () => {
    const fixture = init();
    httpMock.expectOne('/api/projects/1/console/sessions').flush([
      row('1-console-a1b2c3d4', '2026-08-27T10:00:00Z'),
      row('1-console-e5f6a7b8', '2026-08-27T11:00:00Z'),
  ...
  // ?session=<id>. The engine only lists an agent session as open once something has
  // attached to it, so that id was never in the list, the page discarded it, and
  // the user landed in an existing agent session while the new one's worktree was left
  // behind. The "+" now asks with ?new and this page does the minting.
  it('starts a brand-new agent session for a ?new request, alongside the agent sessions already open (#370)', fakeAsync(() => {
  ...
  it('gives a ?new agent session the Settings default agent, with no picker (#219, #370)', fakeAsync(() => {
    TestBed.inject(DefaultAgentStore).set('codex');
    TestBed.inject(Router).navigateByUrl('/projects/1/console?new=1');
    tick();
  ...
  it('starts another agent session when ?new arrives while this page is already showing (#370)', fakeAsync(() => {
    const router = TestBed.inject(Router);
  ...
  it('mints the new agent session under the newly-clicked project, not the one still rendered (#439)', fakeAsync(() => {
    const router = TestBed.inject(Router);
    router.navigateByUrl('/projects/1/console');
    tick();
  ...
  it('ignores a further ?new while one agent session is still being started (#180, #370)', fakeAsync(() => {
    const router = TestBed.inject(Router);
    router.navigateByUrl('/projects/1/console');
    tick();
  ...
  // #537: a project created from a template gets its one seeded agent session opened by
  // this page, without a click, once it is READY -- and never again after the engine
  // has recorded the launch.

  describe('seeded agent session for a templated project (#537)', () => {
    const TEMPLATED = { id: 1, template: 'springboot-angular', templateSeededAt: null };

    it('waits while the project is CLONING, then opens the seeded agent session once it turns READY', fakeAsync(() => {
      TestBed.inject(DefaultAgentStore).set('codex');
      ...
      expect(compiled.querySelector('app-terminal')).toBeFalsy();
      // Nothing is asked of the agent session endpoints while the project is not READY.
    ...
    it('opens it at most once per page instance, even if the project is re-read before the engine recorded the launch', () => {
      ...
      // still unrecorded -- this instance must not mint another seeded agent session.
      fixture.componentRef.setInput('projectId', 2);
      fixture.detectChanges();
      flushProjects([]);
      ...
      httpMock.expectNone('/api/projects/1/console');
      expect(fixture.componentInstance.agentSessions.map((c) => c.seed)).toEqual([null]);
    });
  ...
  it('shows no page title and no past-sessions disclosure -- that list now lives on the project page (#752)', () => {
    ...
    expect(compiled.querySelector('.past-toggle')).toBeFalsy();
  ...
  // page applied them to the matching row of the open-agent-session list. But the engine
  ...
  it('mounts a handed-off reopened session on a project with no open agent session, instead of auto-starting a blank one (#795)', fakeAsync(() => {
    TestBed.inject(Router).navigateByUrl(
    ...
    // Before #795 an empty list auto-started a brand-new agent session here (#256) and the
  ...
  it('mounts the agent session the project page\'s "Open agent" just minted, rather than starting a second one (#795)', fakeAsync(() => {
  ...
  it('renames a tab in place and saves the name against the session (#393)', () => {
    ...
    // immediately, so the header agent sessions widget refetches its rows (#456).
