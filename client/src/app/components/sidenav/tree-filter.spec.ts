...
describe('tree-filter', () => {
  function task(number: number, title: string, state = 'OPEN', hasActiveBranch = false): TreeNode {
    return { number, title, kind: 'TASK', state, hasActiveBranch, children: [] };
  }

  function initiative(number: number, title: string, children: TreeNode[], state = 'OPEN'): TreeNode {
    return { number, title, kind: 'INITIATIVE', state, hasActiveBranch: false, children };
  }
