...
import { ProjectAgentSessionService } from './project-agent-session.service';
...
describe('ProjectAgentSessionService', () => {
  let service: ProjectAgentSessionService;
  ...
  beforeEach(() => {
    ...
    service = TestBed.inject(ProjectAgentSessionService);
  ...
  it('closes one agent session via DELETE /api/projects/{projectId}/console/{sessionId}', () => {
    let completed = false;
  ...
  it('lists the open agent sessions via GET /api/projects/{projectId}/console/sessions', () => {
    const agentSessions = [
      {
    ...
    service.listOpen(1).subscribe((result) => expect(result).toEqual(agentSessions));

    const req = httpMock.expectOne('/api/projects/1/console/sessions');
    expect(req.request.method).toBe('GET');
    req.flush(agentSessions);
  });
  ...
  it('names an agent session tab via PUT .../console/<id>/name, and clears it with an empty name (#393)', () => {
