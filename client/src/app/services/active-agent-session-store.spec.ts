...
describe('ActiveAgentSessionStore', () => {
  beforeEach(() => localStorage.removeItem(STORAGE_KEY));
  afterEach(() => localStorage.removeItem(STORAGE_KEY));

  it('returns null for an issue it has never seen', () => {
    expect(new ActiveAgentSessionStore().get(7)).toBeNull();
  });

  it('remembers the active agent session across instances (i.e. across reloads)', () => {
    new ActiveAgentSessionStore().set(7, '7-main-a1b2c3d4');

    expect(new ActiveAgentSessionStore().get(7)).toBe('7-main-a1b2c3d4');
  });

  it('keeps separate issues independent', () => {
    const store = new ActiveAgentSessionStore();
