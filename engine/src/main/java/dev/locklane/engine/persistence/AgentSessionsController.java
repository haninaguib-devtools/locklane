...
/**
 * Serves every open agent session across all of one project's issues, plus its
 * own project-level agent sessions (#194), that the caller may see (#32's header
...
public class AgentSessionsController {
    ...
    @GetMapping
    public List<String> agentSessions(@PathVariable long projectId, Principal principal) {
     ...
     * visibility rule as {@link #agent sessions}, so this can't be used to reveal an agent session
     * outside the caller's own project. 404 for an agent session id the caller may not see,
     ...
     *
     * <p>The URL is the engine's own proxied path for that agent session (#655),
     * {@code /api/projects/{projectId}/consoles/{id} (a path kept under ADR-112)/ide/}, relative so it resolves
     ...
     * carries the agent session's own worktree as a {@code folder} query parameter (#776):
     * code-server otherwise reopens whichever folder was last opened by *any* agent session
     ...
     * {@code id}'s own worktree rather than another agent session's (#776).
     */
