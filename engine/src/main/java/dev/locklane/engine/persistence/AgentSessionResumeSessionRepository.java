...
/**
 * Durable store of the Claude/Codex resume ids captured from agent session output (#102) —
 * survives a server restart, like {@link WorktreeSessionRepository}. Rows are never
 * deleted when an agent session is closed: outliving the agent session process is the
 * point, so a past conversation stays resumable (#101). One row per distinct
 * (agent session, resume id); seeing the same id again only refreshes {@code captured_at}.
 */
@Repository
public class AgentSessionResumeSessionRepository {

    private final JdbcTemplate jdbcTemplate;

    public AgentSessionResumeSessionRepository(DataSource dataSource) {
    ...
    /** Every resume id captured in this agent session, oldest sighting first. */
    public List<AgentSessionResumeSessionRecord> findByWorktree(String worktreeId) {
    ...
    public List<AgentSessionResumeSessionRecord> findAll() {
        return jdbcTemplate.query(
                "SELECT worktree_id, tool, resume_id, captured_at FROM console_resume_sessions ORDER BY captured_at",
                (rs, rowNum) -> toRecord(rs));
    }

    private static AgentSessionResumeSessionRecord toRecord(java.sql.ResultSet rs) throws java.sql.SQLException {
        return new AgentSessionResumeSessionRecord(
