...
/**
 * Covers #195's done-when as it applies to the engine: a genuinely new agent session
 * opening or an open agent session closing broadcasts {@code consolesChanged} with the
 ...
 */
class SessionRegistryAgentSessionsChangedTest {

    @Test
    void attachingABrandNewAgentSessionBroadcastsAgentSessionsChangedWithItsProjectId(@TempDir Path dbDir,
    ...
    @Test
    void reattachingAnAlreadyLiveAgentSessionBroadcastsNothingFurther(@TempDir Path dbDir, @TempDir Path workDir) {
        EventBroadcaster broadcaster = mock(EventBroadcaster.class);
        SessionRegistry registry = newRegistry(dbDir, broadcaster);
        registry.attach("42-7-slug", workDir);
    ...
    @Test
    void closingAnOpenAgentSessionBroadcastsAgentSessionsChanged(@TempDir Path dbDir, @TempDir Path workDir) {
        EventBroadcaster broadcaster = mock(EventBroadcaster.class);
        SessionRegistry registry = newRegistry(dbDir, broadcaster);
        registry.attach("42-7-slug", workDir);
    ...
    @Test
    void closingAnUnknownAgentSessionBroadcastsNothing(@TempDir Path dbDir) {
        EventBroadcaster broadcaster = mock(EventBroadcaster.class);
        SessionRegistry registry = newRegistry(dbDir, broadcaster);
